...
Enhanced data models for SubliStudio v2 with multi-panel support,
template categorization, round frames, auto-save, etc.
"""
...
class ProductType(Enum):
    ...
    TILE = "Tile"
    CUSHION = "Cushion"
    KEYRING_ROUND = "Keyring (Round)"
    KEYRING_SQUARE = "Keyring (Square)"
...
class TemplateTheme(Enum):
    PLAIN = "Plain"
    COLLAGE_BIRTHDAY = "Collage: Birthday"
    COLLAGE_DIWALI = "Collage: Diwali"
    COLLAGE_HOLI = "Collage: Holi"
    COLLAGE_NEWYEAR = "Collage: New Year"
    COLLAGE_VALENTINE = "Collage: Valentine"
...
class FrameInfo:
    ...
    def from_dict(cls, data: dict) -> "FrameInfo":
        return cls(
            ...
            width=data["width"],
            height=data["height"],
            shape=FrameShape(data.get("shape", "rect")),
            photo_index=data.get("photo_index"),
            photo_scale=data.get("photo_scale", 1.0),
            photo_offset_x=data.get("photo_offset_x", 0),
            photo_offset_y=data.get("photo_offset_y", 0),
        )
...
class DesignJob:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "overlay_effects": self.overlay_effects,
            "text_layers": self.text_layers,
            "output_psd_path": self.output_psd_path,
            "output_png_path": self.output_png_path,
    ...
    def from_dict(cls, data: dict) -> "DesignJob":
        return cls(
            template=TemplateInfo.from_dict(data["template"]) if data.get("template") else None,
            photos=[PhotoItem.from_dict(p) for p in data.get("photos", [])],
            background_path=data.get("background_path"),
            overlay_effects=data.get("overlay_effects", []),
            text_layers=data.get("text_layers", []),
            output_psd_path=data.get("output_psd_path"),
            output_png_path=data.get("output_png_path"),
            last_saved_path=data.get("last_saved_path"),
